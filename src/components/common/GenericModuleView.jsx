...
export const GenericModuleView = ({ title, subtitle, category = 'Management Module' }) => {
  ...
  const columns = [
    ...
    { header: 'Item Name', accessor: 'name', render: (r) => <span className="font-extrabold text-charcoal">{r.name}</span> },
    { header: 'Scope', accessor: 'scope', render: (r) => <span className="font-semibold text-tan-800">{r.scope}</span> },
    { header: 'Category', accessor: 'category' },
    { header: 'Priority', accessor: 'priority', render: (r) => <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-tan-100 text-tan-900 border">{r.priority}</span> },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard title="Total Records" value="48" subtext="Synced Live" icon={Layers} />
        <StatCard title="Active Items" value="42" subtext="Operational" icon={CheckCircle} trend="92% Active" />
        <StatCard title="Pending Review" value="4" subtext="Requires Action" icon={Filter} />
        <StatCard title="System Sync" value="100%" subtext="Real-time Data" icon={RefreshCw} />
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-charcoal-light font-medium">Synced with {currentUser.scope}</span>
        </div>
